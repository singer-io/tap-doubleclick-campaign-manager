...
def transform_field(dfa_type, value):
    ...
    if dfa_type == "boolean":
        ...
        return value == "true" or value == "t" or value == "yes" or value == "y"
...
def sync_report(service, field_type_lookup, profile_id, report_config):
    ...
    with singer.metrics.job_timer("run_report"):
        ...
        while True:
            ...
            if status == "QUEUED":
                LOGGER.info(f"{stream_name}: report_id {report_id} / file_id {report_file_id}  is QUEUED")
                ...
                message = "{}: report_id {} / file_id {} - File processing deadline exceeded ({} secs)".format(
                    stream_name, report_id, report_file_id, status, MAX_RETRY_ELAPSED_TIME
                )
                LOGGER.error(message)
                raise Exception(message)
            ...
            LOGGER.info(
                "{}: report_id {} / file_id {} - File status is {}, sleeping for {} seconds".format(
